...
class MultiNNUNet(NNUNet):
    def __init__(self, *args, **kwargs):
        super(MultiNNUNet, self).__init__(*args, **kwargs)

    def forward(self, x: Tensor):
        encoding = self.encode(x)
        bottleneck = encoding[-1]
        decoding = self.decode(encoding)
        output = self.classifier(decoding)
        return output, bottleneck


class SplitNNUNet(NNUNet):
    def __init__(self, *args, **kwargs):
        super(SplitNNUNet, self).__init__(*args, **kwargs)
